...
func setupRouter(cfg *config.Config, appServices *Services) *gin.Engine {
	...
	if cfg.AgentMode {
		...
		api.NewEnvironmentHandler(apiGroup, appServices.Environment, appServices.Container, appServices.Image, appServices.ImageUpdate, appServices.Updater, appServices.Network, appServices.Volume, appServices.Stack, appServices.Project, appServices.Settings, authMiddleware, cfg)
		return router
	}
	...
	api.NewStackHandler(apiGroup, appServices.Stack, appServices.Project, authMiddleware)
