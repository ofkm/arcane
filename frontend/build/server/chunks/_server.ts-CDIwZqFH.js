...
import { s as startContainer } from './container-service-m5_StWPI.js';
...
const POST = async ({ params }) => {
  ...
  const result = await tryCatch(startContainer(containerId));
  if (result.error) {
    console.error(`API Error starting container ${containerId}:`, result.error);
    const response = {
      success: false,
...
export { POST };
//# sourceMappingURL=_server.ts-CDIwZqFH.js.map
