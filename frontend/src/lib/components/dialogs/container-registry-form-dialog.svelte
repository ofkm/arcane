<script lang="ts">
	...
	let { open = $bindable(false), registryToEdit = $bindable(), onSubmit, isLoading }: ContainerRegistryFormDialogProps = $props();
	...
	// Form validation
	let isFormValid = $derived(internalRegistry.url.trim() !== '' && internalRegistry.username.trim() !== '' && (isEditMode || internalRegistry.token?.trim() !== ''));
</script>
	...
	<Dialog.Content class="sm:max-w-[500px]">
		...
		<form onsubmit={preventDefault(handleSubmit)} class="grid gap-4 py-4">
			<div class="grid gap-2">
				...
				<Input id="registry-url" bind:value={internalRegistry.url} placeholder="e.g., docker.io, ghcr.io, gcr.io, quay.io" required />
			...
			<div class="grid gap-2">
				...
				<Input id="registry-username" bind:value={internalRegistry.username} placeholder="Your registry username" required />
			</div>

			<div class="grid gap-2">
				<Label for="registry-token">Password/Token *</Label>
				<Input type="password" id="registry-token" bind:value={internalRegistry.token} placeholder={isEditMode ? 'Leave empty to keep current token' : 'Your registry password or token'} required={!isEditMode} />
				<p class="text-muted-foreground text-xs">For GitHub, use a Personal Access Token. For Docker Hub, use your password.</p>
			</div>

			<div class="grid gap-2">
				<Label for="registry-description">Description</Label>
				<Textarea id="registry-description" bind:value={internalRegistry.description} placeholder="Optional description for this registry" rows={2} />
			</div>
			...
			<div class="flex items-center justify-between rounded-lg border p-4">
				<div class="space-y-0.5">
					...
					<p class="text-muted-foreground text-sm">Allow HTTP connections (not recommended for production)</p>
				</div>
