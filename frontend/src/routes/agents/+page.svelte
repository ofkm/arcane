...
<div class="container mx-auto px-6 py-8">
	...
	<div class="mb-8 flex items-center justify-between">
		...
		<button onclick={loadAgents} class="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50" disabled={loading}>
			{#if loading}
	...
	<!-- Error Message -->
	{#if error}
		<div class="mb-6 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-red-700 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400">
	...
	{#if agents.length === 0}
		...
		<div class="py-16 text-center">
			...
			<div class="mx-auto max-w-md rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
				...
				<code class="mt-2 inline-block rounded bg-gray-100 px-2 py-1 font-mono text-sm text-gray-800 dark:bg-gray-700 dark:text-gray-200"> http://localhost:3000/agent/register </code>
			</div>
		</div>
	{:else}
		<!-- Agents Grid -->
		<div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
			{#each agents as agent}
				<div class="rounded-xl border border-gray-200 bg-white p-6 transition-shadow hover:shadow-lg dark:border-gray-700 dark:bg-gray-800">
					<!-- Agent Header -->
					<div class="mb-4 flex items-start justify-between">
						<div class="flex items-center gap-3">
							<div class="relative">
								...
								<div class="absolute -top-1 -right-1 h-4 w-4 {getStatusColor(agent)} rounded-full border-2 border-white dark:border-gray-800"></div>
							</div>
							<div>
					...
					<div class="space-y-3">
						...
						<!-- Resource Metrics -->
						{#if agent.metrics}
							<div class="grid grid-cols-2 gap-2 border-t border-gray-100 py-3 dark:border-gray-700">
