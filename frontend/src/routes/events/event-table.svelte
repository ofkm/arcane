	...
	<Card.Root class="border shadow-sm">
		...
		<Card.Content>
			...
			>
				{#snippet rows({ item })}
					<Table.Cell>
						<StatusBadge text={capitalizeFirstLetter(item.severity)} variant={getSeverityBadgeVariant(item.severity)} />
					</Table.Cell>
					<Table.Cell>
					...
					<Table.Cell>
						<DropdownMenu.Root>
							...
							<DropdownMenu.Content align="end">
								<DropdownMenu.Group>
									<DropdownMenu.Item variant="destructive" onclick={() => handleDeleteEvent(item.id, item.title)}>
										<Trash2 class="size-4" />
										Delete
									</DropdownMenu.Item>
								</DropdownMenu.Group>
...
{:else}
	<div class="flex flex-col items-center justify-center px-6 py-12 text-center">
		...
		<p class="text-muted-foreground mt-1 max-w-md text-sm">Events will appear here as they are generated by system activities</p>
	</div>
{/if}
