...
<div class="settings-dashboard px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
	<!-- Header -->
	<div class="mb-6 sm:mb-8">
		<div class="from-background/60 via-background/40 to-background/60 relative overflow-hidden rounded-xl border bg-gradient-to-br p-4 sm:p-6 shadow-sm">
			...
			<div class="relative">
				<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
					<div class="flex items-start gap-3 sm:gap-4">
						...
						<div class="min-w-0">
							<h1 class="text-xl sm:text-2xl font-bold tracking-tight">{m.sidebar_settings()}</h1>
				...
				<!-- Search Bar -->
				<div class="relative mt-4 sm:mt-6 w-full sm:max-w-md">
					...
					>
						<div class="flex items-start justify-between gap-3">
							<div class="flex items-start gap-3 sm:gap-4 min-w-0 flex-1">
								<div class="bg-primary/5 text-primary ring-primary/10 group-hover:bg-primary/10 flex size-10 sm:size-12 items-center justify-center rounded-lg ring-1 transition-colors shrink-0">
									<svelte:component this={category.icon} class="size-5 sm:size-6" />
								</div>
								<div class="min-w-0 flex-1">
									<h3 class="text-sm sm:text-base font-semibold leading-tight">{category.title}</h3>
									<p class="text-muted-foreground mt-1 text-xs sm:text-sm leading-relaxed">{category.description}</p>
								</div>
							</div>
							<ChevronRightIcon class="text-muted-foreground group-hover:text-foreground size-4 transition-colors shrink-0 mt-1" />
						</div>
					</button>
				</Card>
			{/each}
		...
		<div class="space-y-6 sm:space-y-8">
			...
			{#if searchResults.length === 0}
				<div class="text-center py-8 sm:py-12">
					<SearchIcon class="text-muted-foreground mx-auto size-8 sm:size-12 mb-3 sm:mb-4" />
					<h3 class="text-base sm:text-lg font-medium mb-2">No settings found</h3>
