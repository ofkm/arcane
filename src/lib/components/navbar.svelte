...
<!-- Mobile menu button -->
<Button variant="ghost" size="icon" class="md:hidden fixed top-4 right-4 z-50 shadow-sm bg-background" onclick={() => (isOpen = !isOpen)} aria-label={isOpen ? 'Close menu' : 'Open menu'}>
	{#if isOpen}
		<X size={18} />
	{:else}
		<Menu size={18} />
