<script lang="ts">
	...
	let { agents }: { agents: { name: string; logo: typeof IconType; plan: string }[] } = $props();
	const sidebar = useSidebar();

	let activeTeam = $state(agents[0]);
</script>
	...
	<Sidebar.MenuItem>
		<DropdownMenu.Root>
			...
			<DropdownMenu.Content class="w-(--bits-dropdown-menu-anchor-width) min-w-56 rounded-lg" align="start" side={sidebar.isMobile ? 'bottom' : 'right'} sideOffset={4}>
				<DropdownMenu.Label class="text-muted-foreground text-xs">Agents</DropdownMenu.Label>
				{#each agents as agent, index (agent.name)}
					<DropdownMenu.Item onSelect={() => (activeTeam = agent)} class="gap-2 p-2">
						<div class="flex size-6 items-center justify-center rounded-md border">
							<agent.logo class="size-3.5 shrink-0" />
						</div>
						{agent.name}
