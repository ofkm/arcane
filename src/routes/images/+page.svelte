  ...
  async function handlePullImageSubmit(event: {
    ...
    try {
      ...
      const eventSource = new EventSource(
        `/api/images/pull-stream/${encodedImageRef}?tag=${tag}${
  ...
  async function handleDeleteSelected() {
    ...
    const deletePromises = selectedIds.map(async (id) => {
      try {
        ...
        if (image?.inUse) {
          toast.error(
            `Image "${image.repo}:${image.tag}" (${id.substring(
              0,
              12
      ...
      <Card.Header class="px-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <Card.Description>
              View and manage your Docker images
            </Card.Description>
          </div>
          <div class="flex items-center gap-2">
    ...
    <Dialog.Content>
      <Dialog.Header>
        ...
        <Dialog.Description>
          ...
          This action cannot be undone. Images currently used by containers will
          not be deleted.
        </Dialog.Description>
      </Dialog.Header>
  ...
  <Dialog.Root bind:open={isConfirmPruneDialogOpen}>
    <Dialog.Content>
      <Dialog.Header>
        ...
        <Dialog.Description>
          Are you sure you want to remove all unused (dangling) Docker images?
          This will free up disk space but cannot be undone. Images actively
          used by containers will not be affected.
        </Dialog.Description>
      </Dialog.Header>
      <div class="flex justify-end gap-3 pt-6">
        <Button
