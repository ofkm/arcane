...
// Set up path aliases for TypeScript imports
const moduleAlias = require('module-alias');
moduleAlias.addAlias('$lib', path.join(__dirname, 'src/lib'));

async function importServices() {
	const { hybridAgentTasksService } = await import('./src/lib/services/hybrid-agent-tasks-service.js');
	const { agentTasksMigrationService } = await import('./src/lib/services/agent-tasks-migration-service.js');
	const { databaseAgentTasksService } = await import('./src/lib/services/database-agent-tasks-service.js');
	
	return {
		hybridAgentTasksService,
		agentTasksMigrationService,
		databaseAgentTasksService
	};
}
...
async function testAgentTasksMigration() {
	...
	try {
		...
		if (dbAgentTasks.length > 0) {
			...
			const updateResult = await databaseAgentTasksService.updateAgentTaskStatus(
				firstTask.id, 
				'in-progress'
			);
